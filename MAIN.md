...
- **Relay declaratively.** MAIN reads the finalized artifact, relays the role
  ...
  order.
- **Continue to the request's end.** The developer's instruction sets the
  span: `RUN <stage>` or a named agent covers that one stage, and an
  instruction naming the workflow, a stage range, or an end state covers every
  stage within it. MAIN advances through the span, skips a stage current
  evidence shows has no work, and pauses at the span's end or a stage question
  to relay and offer next steps. The developer may direct any stage or command
  at any time.
